...
data class Vector4D(
  ...
  operator fun plusAssign(other: Vector4D) = run { x += other.x; y += other.y; z += other.z; w += other.w }
  operator fun minus(other: Vector4D) = Vector4D(x - other.x, y - other.y, z - other.z, w - other.w)
  operator fun minusAssign(other: Vector4D) = run { x -= other.x; y -= other.y; z -= other.z; w -= other.w }
  operator fun unaryMinus() = Vector4D(-x, -y, -z, -w)
  operator fun times(other: Int) = Vector4D(x * other, y * other, z * other, w * other)
  operator fun timesAssign(other: Int) = run { x *= other; y *= other; z *= other; w *= other }
